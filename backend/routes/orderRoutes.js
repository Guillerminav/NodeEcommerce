...
orderRouter.get('/:id', isAuth, expressAsyncHandler(async (req, res) => {
    ...
    if (order) {
        ...
        res.status(404).send({ message: 'No se encuentra la orden' })
    }
...
orderRouter.put(
    ...
    expressAsyncHandler(async (req, res) => {
        ...
        if (order) {
            ...
            res.send({ message: 'Pago realizado', order: updateOrder })
        } else {
            res.status(404).send({ message: 'No se pudo realizar el pago' })
        }
    })
)
